...
# ==================================
radius = 8
v_data[:] = 4*radius*np.pi/time_end
delta = np.arctan(model.L/radius)
segment = len(t_data)/8
for i in range(t_data.shape[0]):
    x_data[i] = model.xc
    y_data[i] = model.yc
    
    if i < segment or i > 5*segment:
        if model.delta < 0.98*delta:
            w_data[i] = model.w_max*0.988
        else:
            w_data[i] = 0
    elif i >= segment and i <= 5*segment:
        if model.delta > -0.98*delta:
